<template>
  <div
    class="carousel"
    v-on="
      isActive && !blocked && numberOfPages > 1
        ? {
  ...
  >
    <div
      v-if="isActive"
      v-show="showNavigation && length > 1"
      class="carousel-navdots"
    >
      <div
...
export default {
  ...
  computed: {
    navdotStyle() {
      return {
        height: this.navigationConfig.height
          ? this.navigationConfig.height
          : "10px",
        flexBasis: this.navigationConfig.width
          ? this.navigationConfig.width
          : "10px",
        flexGrow: this.navigationConfig.fullWidth ? 1 : 0,
        margin: this.navigationConfig.margin
          ? this.navigationConfig.margin
          : "0 5px",
        borderRadius: this.navigationConfig.borderRadius
          ? this.navigationConfig.borderRadius
          : "50%",
        transition: "all 0.5s",
        cursor: "pointer",
      }
  ...
  methods: {
    ...
    onTouchMove() {
      ...
      if (
        (this.move < 0 && this.maxScrollRight) ||
        (this.move > 0 && this.maxScrollLeft)
      ) {
    ...
    onTouchEnd() {
      if (Math.abs(this.move) > this.sensitivity) {
        ...
        Math.abs(this.move) < this.sensitivity &&
        Math.abs(this.move) > 1
      ) {
        this.animateCarousel()
      }
